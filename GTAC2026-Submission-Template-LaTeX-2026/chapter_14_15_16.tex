...
\title{{Physical AI and Vertical Software: Bridging AMD's Softpower Gap - From the Voice AI Perspective}}
...
The pattern from Table~\ref{tab:compat} is clear: the two heaviest compute layers (LLM serving and TTS synthesis) are solved; two narrower but load-bearing pieces---streaming ASR and HuggingFace Text Embeddings Inference (TEI) for RAG-augmented agents---remain open.
...
Despite competitive silicon, AMD is absent from the ecosystem surface that drives the adoption decision. Table~\ref{tab:nim_gap} shows the NIM-layer gap: ten ecosystem positions where NVIDIA is present and AMD is not. The gap is not silicon but its the least resistant path-to-silicon; it is containers, plugins, blueprints, and marketplace slots.
...
SI certification (TCS, Infosys...) & SI certification program & \textbf{Absent} \\
...
\begin{itemize}
  \item \textbf{Synthetic training data for low-resource languages.} NVIDIA's ecosystem includes speech data generation pipelines, models trained on NVIDIA infrastructure carry a lifecycle dependency that predates any inference decision---AMD has no equivalent capability or academic training partnership in these markets~\cite{indic_asr,sarvam_asr}.
\end{itemize}
...
\textbf{The structural position.}
The Physical AI Vertical Software (PAVS) team sits exactly at the intersection where AMD's softpower gap is most visible and most closeable: between AMD's AI Group (AIG) model outputs and lighthouse customers who validate and deploy those models in real environments. Every time PAVS converts a raw AIG model into production-ready vertical software for a lighthouse customer, it is doing---organically---exactly what NVIDIA's platform teams do intentionally: turning hardware capability into software that developers and customers can adopt without understanding the hardware beneath it. The six steps below are the explicit form of what PAVS is already doing implicitly.
...
Voice AI is the lens, not the limit. The same softpower deficit appears in robotics (no Isaac equivalent), in industrial inspection (no Metropolis equivalent), in healthcare imaging (no certified clinical inference pipeline), and in automotive (no JetPack for embedded x86). Every domain where PAVS works with a lighthouse customer is a domain where AMD's softpower gap is real and where vertical software delivery directly addresses it:

\begin{itemize}
  \item Each lighthouse deployment that produces a reusable pipeline is a step toward an AMD-native software ecosystem.
  \item Each model the Physical AI SDK supports is a model that no longer requires NVIDIA tooling to deploy on AMD hardware.
  \item Each robotics pipeline that runs on Ryzen AI APU is a reference design that an OEM, SI, or ISV can build on.
\end{itemize}

The window is open. NVIDIA's Jetson ecosystem deepens every quarter; its NIM integrations expand; its SI certifications multiply. The cost of displacing those relationships rises with each passing quarter. The softpower gap is not a hardware problem---it cannot be closed by a new chip. It is a software and ecosystem problem, which means it is exactly the kind of problem that a team doing vertical software delivery is positioned to solve.
