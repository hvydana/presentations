...
**279 ecosystem components enumerated and checked. 194 are gaps. 5 unresolved.**
...
#### [P0 · 54] No AMD-maintained catalog of ROS 2 semantic segmentation nodes with benchmarked models

- **Component / trigger:** isaac_ros_image_segmentation (NVIDIA) — Semantic image segmentation nodes
- **AMD status:** `partial` — partial / in progress (nearest: AMD ROS 2 Perception Node accelerated application on ROCm/Vitis AI)
- **Who is blocked:** Perception engineers porting segmentation pipelines off Isaac ROS
- **Action:** Publish segmentation model zoo entries with MIGraphX and Ryzen AI latency tables in the Robotics Software Suite
- **Scores:** openness 3 · fit 3 · leverage 3 · evidence 2 · confidence med
- **Evidence:** https://newsroom.amd.com/news/aai-2026-robotics-partner-network/
- **Component ref:** https://github.com/orgs/NVIDIA-ISAAC-ROS/repositories?type=all
...
#### [P1 · 24] No 6D object pose estimation path on AMD; FoundationPose and CenterPose are CUDA-bound

- **Component / trigger:** isaac_ros_pose_estimation (NVIDIA) — 6D object pose estimation nodes
- **AMD status:** `none` — no AMD path
- **Who is blocked:** Bin-picking and manipulation teams evaluating AMD for grasping
- **Action:** Port one open 6D pose model (FoundationPose or MegaPose) to ROCm/MIGraphX and publish a demo
- **Scores:** openness 2 · fit 2 · leverage 3 · evidence 2 · confidence med
- **Evidence:** https://developer.nvidia.com/isaac/ros
- **Component ref:** https://github.com/orgs/NVIDIA-ISAAC-ROS/repositories?type=all
...
#### [P1 · 18] No GPU or FPGA accelerated AprilTag fiducial detector on AMD - only CPU apriltag_ros

- **Component / trigger:** isaac_ros_apriltag (NVIDIA) — GPU fiducial marker detection
- **AMD status:** `none` — no AMD path
- **Who is blocked:** Warehouse docking calibration and marker-based localization users
- **Action:** Port AprilTag detection to HIP and publish fps versus the CPU baseline
...
#### [P2 · 12] No AMD accelerated component-graph runtime though NVIDIA is itself sun-setting GXF

- **Component / trigger:** GXF (NVIDIA) — Accelerated component graph framework
- **AMD status:** `none` — no AMD path
- **Who is blocked:** Few - GXF is being removed from NITROS so downstream exposure is shrinking
- **Action:** Do not chase GXF - invest the effort in ROS 2 type adaptation instead
...
#### [P2 · 8] No cross-process or cross-container GPU-memory handle bridge for non-accelerated ROS 2 nodes

- **Component / trigger:** isaac_ros_nitros_bridge (NVIDIA) — Accelerated ROS bridge
- **AMD status:** `none` — no AMD path
- **Who is blocked:** Teams mixing legacy ROS 2 nodes with accelerated AMD nodes across containers
- **Action:** Prototype a DMA-BUF handle bridge node so unaccelerated nodes avoid host copies
- **Scores:** openness 2 · fit 2 · leverage 2 · evidence 1 · confidence low
- **Evidence:** https://github.com/NVIDIA-ISAAC-ROS/isaac_ros_nitros
- **Component ref:** https://github.com/orgs/NVIDIA-ISAAC-ROS/repositories?type=all

#### [P2 · 8] No freespace/ground-plane segmentation package for navigation on AMD

- **Component / trigger:** isaac_ros_freespace_segmentation (NVIDIA) — Freespace segmentation for navigation
- **AMD status:** `none` — no AMD path
- **Who is blocked:** AMR navigation teams wanting learned drivable-space masks
- **Action:** Low priority; add as a model variant once the segmentation node catalog exists
...
#### [P3 · 4] No depth-based obstacle field segmentation node on AMD

- **Component / trigger:** isaac_ros_depth_segmentation (NVIDIA) — Depth obstacle field segmentation
- **AMD status:** `none` — no AMD path
- **Who is blocked:** Teams replicating Bi3D-style binary depth obstacle detection
- **Action:** Deprioritize; cover via generic depth plus segmentation pipeline instead of a dedicated node
- **Scores:** openness 1 · fit 2 · leverage 1 · evidence 2 · confidence med
- **Evidence:** https://developer.nvidia.com/isaac/ros
- **Component ref:** https://github.com/orgs/NVIDIA-ISAAC-ROS/repositories?type=all
...
#### [P0 · 54] No ROCm equivalent to cuRobo GPU motion planning

- **Component / trigger:** cuRobo (NVIDIA) — GPU parallel motion planning
- **AMD status:** `none` — no AMD path (nearest: rocRobo (embryonic))
- **Who is blocked:** manipulation teams needing GPU-parallel collision-free planning
- **Action:** Identify rocRobo maintainers and roadmap; evaluate AMD sponsorship timed with the Warp PR merges
- **Scores:** openness 3 · fit 3 · leverage 3 · evidence 2 · confidence med
- **Evidence:** prior:flow_graph/robotics-ecosystem/gap_register.md#GAP-14
- **Component ref:** https://github.com/NVlabs/curobo

#### [P0 · 54] No portable GPU motion planning library outside cuRobo

- **Component / trigger:** (whitespace) open GPU motion planning library (NONE) — GPU-accelerated collision-free motion planning (non-NVIDIA)
- **AMD status:** `none` — no AMD path (nearest: rocRobo (embryonic))
- **Who is blocked:** the whole non-NVIDIA manipulation ecosystem
- **Action:** Back rocRobo or a portable alternative; this is the capability-level view of the cuRobo gap
- **Scores:** openness 3 · fit 3 · leverage 3 · evidence 2 · confidence med
- **Evidence:** prior:flow_graph/robotics-ecosystem/gap_register.md#GAP-14
- **Component ref:** derived:grid-sweep

#### [P0 · 36] cuMotion productized motion generation has no AMD counterpart

- **Component / trigger:** cuMotion (NVIDIA) — CUDA motion generation library
- **AMD status:** `none` — no AMD path
- **Who is blocked:** industrial arm integrators
- **Action:** Treat as downstream of the rocRobo/cuRobo decision rather than a separate build
- **Scores:** openness 3 · fit 2 · leverage 3 · evidence 2 · confidence med
- **Evidence:** prior:flow_graph/robotics-ecosystem/gap_register.md#GAP-14
- **Component ref:** https://github.com/nvidia-isaac/cumotion

#### [P0 · 36] No AMD-accelerated MoveIt 2 motion planning node

- **Component / trigger:** isaac_ros_cumotion (NVIDIA) — ROS 2 arm motion planning nodes
- **AMD status:** `none` — no AMD path
- **Who is blocked:** ROS 2 manipulation stacks
- **Action:** Package whatever GPU planner AMD backs as a MoveIt 2 plugin, mirroring isaac_ros_cumotion
- **Scores:** openness 3 · fit 2 · leverage 3 · evidence 2 · confidence med
- **Evidence:** prior:flow_graph/robotics-ecosystem/gap_register.md#GAP-14
- **Component ref:** https://github.com/orgs/NVIDIA-ISAAC-ROS/repositories?type=all
...
#### [P1 · 18] No ROCm-runnable 6D pose foundation model for novel-object manipulation

- **Component / trigger:** FoundationPose (NVIDIA) — 6D pose estimation foundation model
- **AMD status:** `none` — no AMD path
- **Who is blocked:** bin-picking and manipulation teams on AMD GPUs
- **Action:** Port the CUDA extensions FoundationPose depends on (nvdiffrast, custom raycasting) to HIP and publish a ROCm container
- **Scores:** openness 1 · fit 2 · leverage 3 · evidence 3 · confidence med
- **Evidence:** https://github.com/NVlabs/FoundationPose
- **Component ref:** https://github.com/NVlabs/FoundationPose
