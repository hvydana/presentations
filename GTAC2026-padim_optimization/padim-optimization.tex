...
image-level AUROC on the MVTec AD benchmark~\cite{bergmann2019mvtec}---without
...
\begin{align}
  ...
  \mathbf{tmp}_{b,c,i}  &= \sum_{d}\, C^{-1}_{c,d,i}\;\mathbf{diff}_{b,d,i}
    \;\; [\texttt{einsum(`cdi,bdi->bci')}] \label{eq:tmp}\\
  d_{b,i}               &= \sqrt{\sum_{c}\, \mathbf{diff}_{b,c,i}\;\mathbf{tmp}_{b,c,i}}
    \;\; [\texttt{einsum(`bci,bci->bi')}] \label{eq:dist}
\end{align}
%
...
\end{itemize}
%
Mixed-precision execution (BF16, FP16) is enabled at the session level by
casting model weights prior to the ONNX export, ensuring the MIGraphX compiler
sees a clean half-precision graph without inserted up/down-cast nodes.
...
Supports CPU and CUDA/ROCm GPU.  Intended for development and hardware without
ONNX Runtime.
...
registry loaded from a configurable directory, and covariance-mode selection via
the \texttt{use\_diagonal\_cov} flag at startup.
...
\node[inf, below=of ec] (mah) {\textbf{Vectorized Mahalanobis}\\ 2$\times$\texttt{einsum} or diagonal\\ all 3{,}136 positions parallel};
...
\node[left=5mm of mah, text width=0.85in, align=center, font=\tiny]
  {\emph{pre-loaded}\\ $\mu,\;\mathbf{C}^{-1}$ (or $\mathbf{var\_inv}$)};
\end{tikzpicture}
...
previous ones.  Baseline uses the reference SciPy loop; subsequent rows add
pre-computed $\Sigma^{-1}$, vectorized einsum, ONNX GPU execution, and diagonal
covariance in sequence.}
\label{tab:ladder}
\footnotesize
...
GPU).  Together they multiply rather than add, giving the 572$\times$ headline.
...
CPU variants use ONNX Runtime CPUExecutionProvider.
Baseline (reference SciPy loop) shown for comparison.}
\label{tab:benchmark}
\footnotesize
...
The surprising finding that discarding off-diagonal covariance terms preserves
AUROC exactly warrants scrutiny.  PaDiM's dimensionality reduction step applies
...
A formal analysis of the random-projection-induced decorrelation, corroborated
...
($100\!\times\!3136\!\times\!4$ bytes), a 15$\times$ reduction that is directly
...
covariance computation to training time; (2) vectorize the distance computation
with einsum or equivalent tensor contractions; (3) export to ONNX and execute
