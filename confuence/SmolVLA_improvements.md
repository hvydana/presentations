...
style: |
  ...
  .title-blue   { color: #3498db !important; font-size: 2.0em; margin-top: 0.3em; }
  .title-green  { color: #4caf50 !important; font-size: 1.6em; margin-top: 0.2em; font-weight: bold; }
  .title-green strong { color: #4caf50 !important; }
  .title-sub    { color: #aaaaaa !important; font-size: 0.85em; margin-top: 1em; }
  .team-pavs    { color: #ff9800 !important; font-size: 1.3em; font-weight: bold; margin-top: 1.5em; }
---
...
<h2 class="title-blue">Is Acceleration New Accuracy?</h2>

<h3 class="title-green">Acceleration ≡ Reactivity ≡ Task Accuracy</h3>

<p class="title-sub">2 → 13–15 inf/s &nbsp;|&nbsp; MAE 6 → ~3 (usable limit ~3.5)</p>

<p class="team-pavs">PAVS</p>
...
- **torch.compile:** fused operators into a single graph → optimized GPU kernels, no Python overhead per layer (but needed code rewrite).
...
## Accuracy & Smoothness Gains

<div class="columns">
<div>

**Data + fine-tune → MAE 6 → ~3**

| Stage | MAE |
|---|:-:|
| Past delivery model            | **6.0** |
| + Combined dataset & fine-tune | **~3.0** |
| Usable-limit threshold         | **~3.5** |

→ <span class="success">~50% MAE reduction</span> — now <strong>under the ~3.5 usability limit</strong>.
...
- Continuous action space + limited data → visible **shake/jitter** on the robot.
- Added lightweight **post-processing smoother** on the action stream.
- Smooth, deployable motion — **no retraining needed**.

</div>
</div>

<div class="highlight-box">
<span class="success">Net result:</span> real-time, smooth, accurate gesture-mimic stack on SmolVLA + LeRobot.
</div>